...
#define AMSVC_SYSTEM_PROFILER       CFSTR("com.apple.mobile.system_profiler")
	...
	mach_error_t AMDeviceStartService(struct am_device *device, CFStringRef 
									  service_name, struct afc_connection **handle, unsigned int *
									  unknown);
	...
	afc_error_t AFCDirectoryOpen(struct afc_connection *conn, const char *path,
	...
	afc_error_t AFCDirectoryCreate(struct afc_connection *conn, const char *dirname);
	afc_error_t AFCRemovePath(struct afc_connection *conn, const char *dirname);
	afc_error_t AFCRenamePath(struct afc_connection *conn, const char *from, const char *to);
	...
	afc_error_t AFCFileInfoOpen(struct afc_connection *conn, const char *path, struct
								afc_dictionary **info);
